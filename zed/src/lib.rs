//! The Zed side of Secrets-LE.
//!
//! This crate holds no extraction logic. It resolves `secrets-le-mcp` from npm and
//! hands Zed a command that launches it over stdio — the same server the VSIX
//! bundles and the same one `npx secrets-le-mcp` runs. Keeping it a launcher is
...
const PACKAGE_NAME: &str = "secrets-le-mcp";
const SERVER_PATH: &str = "node_modules/secrets-le-mcp/server.js";

struct SecretsLEExtension;
...
impl zed::Extension for SecretsLEExtension {
    fn new() -> Self {
        Self
    }
...
zed::register_extension!(SecretsLEExtension);
