...
impl TempTree {
    pub(crate) fn new(name: &str) -> Self {
        ...
        let root =
            std::env::temp_dir().join(format!("secrets-le-{name}-{}-{unique}", std::process::id()));
    ...
    /// stays relative.
    ///
    /// Unused by this crate's own tests today — the walker never follows
    /// links — and kept because `tests/scenarios.rs` builds link cases
    /// with the same helper shape.
    #[cfg(all(unix, test))]
    #[cfg_attr(test, expect(dead_code, reason = "used by the scenario tier"))]
