 ...
 * `npx secrets-le-mcp` does, minus the registry — so it catches the failures a
...
setTimeout(() => {
	...
	if (found.length === 0) {
		fail('the installed server found no secret in a document containing one');
	}
	// Proves absence, not presence: a leak would pass any find-the-value check.
	if (JSON.stringify(found).includes('wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY')) {
		fail(
			'the installed server returned the secret value',
