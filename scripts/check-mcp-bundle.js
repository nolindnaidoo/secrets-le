...
setTimeout(() => {
	...
	if (init?.serverInfo?.name !== 'secrets-le') {
	...
	if (call?.isError !== false) {
		fail('detect_secrets reported an error', JSON.stringify(call));
	}
	const found = call?.structuredContent?.data?.secrets ?? [];
	if (found.length === 0) {
		fail('detect_secrets found nothing in a document containing a known key');
	}

	// The inverse of every other gate in the family: this one proves a value is
	// ABSENT. A server that leaked the credential would sail through any
	// did-it-find-something check, so absence is what has to be asserted.
	const serialised = JSON.stringify(found);
	if (serialised.includes('wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY')) {
		fail(
			'detect_secrets returned the secret value',
			'previews must be masked — see src/utils/mask.ts',
		);
	}
	if (!serialised.includes('AWS_SECRET_ACCESS_KEY')) {
		fail(
			'detect_secrets did not report the key name',
