...
- **nls catalogues stay in key-parity:** all 12 locale files in `src/i18n/` carry exactly the keys of the committed root `package.nls.json`. This repo used to say it was English-only long after it shipped twelve translations, and the hub site repeated it — never restate the locale count as prose when the catalogues are right there to count.
